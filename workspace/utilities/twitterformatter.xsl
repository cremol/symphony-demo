        ...
        <li class="tweet">
            <p>
                <xsl:call-template name="linkahashify">
                    <xsl:with-param name="tweet" select="substring-after(title, ':')"/>
                </xsl:call-template>
                <span class="twitdate opacity">
                    <xsl:value-of select="substring-before(pubDate, '+0000')"/>
                </span>
            </p>
            <a href="{link}" target="_blank" title="go to tweet on twitter" class="twitlink"></a>
        </li>
    </xsl:template>
